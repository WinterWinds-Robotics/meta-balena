...
do_run_engine() {
    ...
    exec env PATH="${HOSTTOOLS_DIR}:${PATH}" XDG_RUNTIME_DIR=${ENGINE_DIR} balenad-rootless.sh --experimental --pidfile ${ENGINE_PIDFILE} -H ${DOCKER_HOST} > ${WORKDIR}/temp/log.balenad-rootless-run-${BB_CURRENTTASK} 2>&1 &
